...
<ol>
  <li>Download <a href="https://github.com/99designs/aws-vault/releases">aws-vault</a> and put it in your PATH. Since it's a Go app, it's a single self-contained binary which is convenient. If you're running Arch Linux, it's also in the <a href="https://aur.archlinux.org/packages/aws-vault">AUR</a></li>
  <li>To add your credentials: <br />
    <blockquote>
$ aws-vault add my-profile<br />
...
Enter passphrase to unlock /home/carlo/.awsvault/keys:
  ...
  <li>To use the credentials: <br />
    <blockquote>
$ aws-vault exec personal ./my-script-that-needs-aws-creds<br />
Enter passphrase to unlock /home/carlo/.awsvault/keys:
    </blockquote>
  </li>
</ol>
...
<ul>
  <li><strong>--session-ttl</strong> - Defaults to 4h but for increased security, you can reduce that time. Conversely, if you get tired of recreating your <code>aws-vault</code> session, you can increase the time, though it increases the amount of time a bad guy has to use your creds if your machine gets stolen.</li>
  <li><strong>--mfa-token</strong> - If you have <a href="http://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_mfa_enable_virtual.html">multi-factor auth</a> enabled on your IAM account, you can pass those tokens into <code>aws-vault</code></li>
</ul>
TEXT
)
